...
デーモン起動時に使用可能なコマンドライン引数は以下の通りです：

| 引数名 | 型 | デフォルト値 | 説明 |
| :--- | :---: | :---: | :--- |
| `-mode` | string | `client` | 動作モード。`client` (クライアント側でTCPインターセプト) または `server` (サーバー側でUDP待受・TCP終端) |
| `-listen` | string | `:10080` | クライアントモード時はTCP透過プロキシの待受アドレス。サーバーモード時はUDPカプセル化パケットの待受アドレス。 |
| `-routes` | string | `routes.yaml` | クライアントモード時、オリジナル宛先TCPアドレスとServer-PEP UDPアドレスのマッピングを定義する設定ファイルのパス。 |
| `-mtu` | int | `1200` | UDPのMTUサイズ (パケットペイロードの最大サイズ)。100〜1500の範囲で指定。 |
| `-bandwidth` | int | `128000` | リンクの最大帯域幅制限 (bps)。例: `128000` は 128kbps。`0` でトークンバケットシェーパを無効化。 |
| `-fec-k` | int | `10` | FECデータシャード数 $K$。1ブロックを構成するデータパケットの数。 |
| `-fec-m` | int | `3` | FEC最大パリティシャード数 $M$。1ブロックあたりの最大パリティ数（動的適応時の上限値）。 |
| `-idle-timeout` | int | `300` | セッションが無通信のまま自動クリーンアップされるまでのタイムアウト時間（秒）。 |
| `-redis-addr` | string | `:6379` | 内蔵 Redis (RESP) 監視サーバーの待受アドレス。空文字列を指定すると無効化されます。 |
| `-http-addr` | string | `:8080` | 内蔵 HTTP/HTTPS 監視サーバーの待受アドレス。空文字列を指定すると無効化されます。 |
| `-http-cert` | string | `""` | HTTPS監視用SSL証明書ファイル (.crt) のパス。指定するとHTTPSが有効化されます。 |
| `-http-key` | string | `""` | HTTPS監視用SSL秘密鍵ファイル (.key) のパス。証明書とともに指定する必要があります。 |
...
## 内蔵 HTTP/HTTPS インターフェースによるリアルタイムモニタリング

HTTP 監視サーバー（デフォルトポート `:8080` または `-http-addr` で指定）は、現在アクティブなすべてのセッション統計情報を一括で JSON 形式で返却するシンプルな GET API エンドポイントを提供します。

### 1. HTTP でのクエリ例
```bash
$ curl http://127.0.0.1:8080/stats
```
またはルートパスへのアクセスでも同様の情報を取得できます：
```bash
...
証明書と鍵を指定して起動した場合：
```bash
$ ./tcp-pep-daemon -http-addr :8443 -http-cert cert.pem -http-key key.pem
```
この場合、クライアントは HTTPS で接続する必要があります（自己署名証明書などの場合は必要に応じて `-k` / `--insecure` を付与してください）：
```bash
